...
pub  fn get_database_location(database_configuration : &DatabaseConfiguration) -> PathBuf {
    Path::new(&database_configuration.location).join(&format!("Database_{}.csv", database_configuration.name))
}
...
pub mod builder {
    ...
    use crate::model::primitives::person::Person;
    use crate::model::remote::server::ServerConfiguration;
    use crate::model::state::config::{AbrechnungsConfiguration, BackupConfiguration, Configuration, DatabaseConfiguration, DesignConfiguration, ErfassungsConfiguration, UserConfiguration};
