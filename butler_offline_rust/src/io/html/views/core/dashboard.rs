...
fn map_to_template(view_result: DashboardViewResult) -> DashboardTemplate {
    DashboardTemplate {
        zusammenfassung_monatsliste: JSONStringList::new(view_result.zusammenfassung_monatsliste.iter().map(|x| x.monat.clone()).collect()),
        zusammenfassung_einnahmenliste: JSONBetragList::new(view_result.zusammenfassung_einnahmenliste.iter().map(|x| x.clone()).collect()),
        zusammenfassung_ausgabenliste: JSONBetragList::new(view_result.zusammenfassung_ausgabenliste.iter().map(|x| x.clone()).collect()),
        ausgaben_des_aktuellen_monats: view_result.ausgaben_des_aktuellen_monats.iter().map(|x| BuchungTemplate {
            index: x.index,
...
mod tests {
    ...
    fn test_map_to_template() {
        let view_result = DashboardViewResult {
            ...
            zusammenfassung_einnahmenliste: vec![Betrag::new(Vorzeichen::Positiv, 10, 10), Betrag::new(Vorzeichen::Positiv, 20, 20)],
            zusammenfassung_ausgabenliste: vec![Betrag::new(Vorzeichen::Positiv, 30, 30), Betrag::new(Vorzeichen::Positiv, 40, 40)],
        ...
        assert_eq!(format!("{}", template.zusammenfassung_monatsliste), "[\"Januar\",\"Februar\"]");
        assert_eq!(format!("{}", template.zusammenfassung_einnahmenliste), "[\"10.10\",\"20.20\"]");
        assert_eq!(format!("{}", template.zusammenfassung_ausgabenliste), "[\"30.30\",\"40.40\"]");
