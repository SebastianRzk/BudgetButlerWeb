...
pub fn map_to_template(view_result: AddGemeinsameBuchungViewResult) -> AddGemeinsameBuchungenTemplate {
    ...
    AddGemeinsameBuchungenTemplate {
        ...
        personen: Select::new(view_result.personen.iter().map(|x| x.person.clone()).collect(), Some(view_result.default_item.person.person.clone())),
    }
}
...
#[cfg(test)]
mod tests {
    use crate::budgetbutler::pages::gemeinsame_buchungen::add_gemeinsame_buchung::{AddGemeinsameBuchungViewResult, DefaultItem, LetzteErfassung};
