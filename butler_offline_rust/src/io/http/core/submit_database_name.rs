...
pub async fn submit(
    ...
    create_database_backup(&database_guard, &config.backup_configuration, today(), now(), "before_rename");
    ...
    *database_guard = refreshed_database;
    create_database_backup(&database_guard, &result.new_config.backup_configuration, today(), now(), "after_rename");
