...
pub fn read_sparbuchungen(sorted_file: &SortedFile) -> Vec<Sparbuchung> {
    sorted_file.sparbuchungen.iter().map(|l| read_sparbuchung(l.into())).collect()
}
...
mod tests {
    ...
    fn test_read_dauerauftraege() {
        ...
        assert_eq!(sparbuchung_eins.wert, betrag(Vorzeichen::Negativ, 123, 12));
        ...
        assert_eq!(sparbuchung_zwei.wert, betrag(Vorzeichen::Negativ, 123, 13));
