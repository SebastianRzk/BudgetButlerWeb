...
use crate::io::disk::einzelbuchungen::einzelbuchung_reader::read_einzelbuchung;
use crate::model::einzelbuchung::Einzelbuchung;

pub fn read_einzelbuchungen(sorted_file: &SortedFile) -> Vec<Einzelbuchung> {
    sorted_file.einzelbuchungen.iter().map(|l| read_einzelbuchung(l.into())).collect()
}
...
mod tests {
    ...
    fn test_read_einzelbuchungen() {
        ...
        assert_eq!(einzelbuchungen[0].betrag, betrag(Vorzeichen::Negativ, 123, 12));
        ...
        assert_eq!(einzelbuchungen[1].betrag, betrag(Vorzeichen::Negativ, 123, 13));
    }
}