use crate::io::disk::diskrepresentation::file::{File, SortedFile, DAUERAUFTRAEGE_HEADER, DAUERAUFTRAG_ORDER_HEADER, DEPOTAUSZUEGE_HEADER, DEPOTWERTE_HEADER, EINZELBUCHUNGEN_HEADER, GEMEINSAME_BUCHUGEN_HEADER, ORDER_HEADER, SPARBUCHUNGEN_HEADER, SPARKONTOS_HEADER, *};
...
pub fn sort_file(file: File) -> SortedFile {
    ...
    SortedFile {
        einzelbuchungen: result.get(&Modus::Einzelbuchungen).unwrap_or(&empty_vec).clone(),
        dauerauftraege: result.get(&Modus::Dauerauftraege).unwrap_or(&Vec::new()).clone(),
        gemeinsame_buchungen: result.get(&Modus::GemeinsameBuchungen).unwrap_or(&Vec::new()).clone(),
        sparbuchungen: result.get(&Modus::Sparbuchungen).unwrap_or(&Vec::new()).clone(),
...
";
    ...
    pub fn test_storter() {
        ...
        assert_eq!(sorted_file.einzelbuchungen.get(0).unwrap().line, "2024-01-01,NeueKategorie,EinName,-123.12");

        assert_eq!(sorted_file.dauerauftraege.len(), 1);
        assert_eq!(sorted_file.dauerauftraege.get(0).unwrap().line, "2024-03-03,2024-02-02,EineKategorie,EinName,monatlich,-123.12");

        assert_eq!(sorted_file.gemeinsame_buchungen.len(), 1);
        assert_eq!(sorted_file.gemeinsame_buchungen.get(0).unwrap().line, "2024-04-04,EineKategorie,EinName,-123.12,EinePerson");

        assert_eq!(sorted_file.sparbuchungen.len(), 1);
        assert_eq!(sorted_file.sparbuchungen.get(0).unwrap().line, "2024-05-05,EinName,-123.12,Manueller Auftrag,MeinKonto");

        assert_eq!(sorted_file.sparkontos.len(), 1);
        assert_eq!(sorted_file.sparkontos.get(0).unwrap().line, "MeinDepot,Depot");

        assert_eq!(sorted_file.depotwerte.len(), 1);
        assert_eq!(sorted_file.depotwerte.get(0).unwrap().line, "MeinBeispielDepotwert,ETF999,ETF");
