...
mod tests {
    ...
    use crate::model::database::sparkonto::builder::any_konto;
    use crate::model::database::sparkonto::Kontotyp;
    use crate::model::primitives::name::Name;
    use crate::model::state::non_persistent_application_state::KontoChange;
    ...
    pub fn test_handle_view_with_edit_index() {
        let database = generate_database_with_sparkontos(vec![any_konto()]);
        let changes = vec![];
        let context = AddKontoContext {
            database: &database,
            edit_buchung: Some(1),
        ...
        assert_eq!(result.default_item.name, any_konto().name);
        assert_eq!(result.default_item.typ, any_konto().kontotyp);
    ...
    pub fn test_handle_view_should_preset_changes() {
        ...
        assert_eq!(result.letzte_erfassungen[0].name, any_konto().name);
        assert_eq!(result.letzte_erfassungen[0].typ, any_konto().kontotyp);
    }
}
