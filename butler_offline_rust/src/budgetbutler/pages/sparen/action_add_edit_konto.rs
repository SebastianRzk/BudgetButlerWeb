...
pub fn submit_sparkonto(context: SubmitKontoContext) -> RedirectResult<KontoChange> {
    ...
    if let Some(index) = context.edit_index {
        icon = PENCIL;
        neue_sparkontos = context.database.sparkontos
            .change()
...
mod tests {
    ...
    fn test_submit_sparkonto() {
        ...
        assert_eq!(result.result.changed_database.sparkontos.select().count(), 1);
        assert_eq!(result.result.changed_database.sparkontos.get(0).value, Sparkonto{
            name: any_name(),
            kontotyp: Kontotyp::Sparkonto,
        });

        assert_eq!(result.change.icon, PLUS.as_fa.to_string());
        assert_eq!(result.change.name, any_name());
