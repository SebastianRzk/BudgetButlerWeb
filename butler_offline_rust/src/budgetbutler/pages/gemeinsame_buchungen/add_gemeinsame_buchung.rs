...
pub fn handle_view(context: AddGemeinsameBuchungContext) -> AddGemeinsameBuchungViewResult {
    ...
    let result = AddGemeinsameBuchungViewResult {
        ...
        action_headline,
        default_item,
        personen: vec![context.user_configuration.self_name.clone(), context.user_configuration.partner_name.clone()],
        kategorien: calc_kategorien(&context.database.einzelbuchungen, context.extra_kategorie),
        action_title,
        letzte_erfassungen: context.gemeinsame_buchungen_changes.iter().map(|change| LetzteErfassung {
...
#[cfg(test)]
mod tests {
    use crate::budgetbutler::pages::gemeinsame_buchungen::add_gemeinsame_buchung::{handle_view, AddGemeinsameBuchungContext};
    ...
    use crate::model::state::non_persistent_application_state::GemeinsameBuchungChange;
    use crate::model::state::persistent_application_state::builder::{generate_database_with_einzel_und_gemeinsamen_buchungen, generate_database_with_gemeinsamen_buchungen, generate_empty_database};

    #[test]
    pub fn test_handle_view_without_edit_index() {
        let database = generate_database_with_einzel_und_gemeinsamen_buchungen(
            vec![any_einzelbuchung()],
            vec![
            GemeinsameBuchung{
        ...
        assert_eq!(result.kategorien, vec![any_kategorie(), kategorie("test_kategorie")]);
    ...
    pub fn test_handle_view_with_edit_index() {
        ...
        assert_eq!(result.default_item.person, Person::new("test_person".to_string()));
        assert_eq!(result.default_item.name, name("test_name"));
        assert_eq!(result.default_item.kategorie, kategorie("test_kategorie"));
        assert_eq!(result.default_item.wert, Betrag::new(Vorzeichen::Negativ, 10, 0));
