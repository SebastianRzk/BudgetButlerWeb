...
mod tests {
    ...
    fn test_delete_dauerauftrag() {
        let database = generate_database_with_gemeinsamen_buchungen(vec![any_gemeinsame_buchung()]);
        ...
        assert_eq!(result.change.betrag, any_gemeinsame_buchung().betrag);
        assert_eq!(result.change.kategorie, any_gemeinsame_buchung().kategorie);
        assert_eq!(result.change.name, any_gemeinsame_buchung().name);
        assert_eq!(result.change.person, any_gemeinsame_buchung().person);
    }
}
