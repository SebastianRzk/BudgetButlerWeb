...
pub fn read_einzelbuchungen(lines: Vec<Line>) -> Vec<Einzelbuchung>{
    lines.into_iter().map(|line| (&line).into()).map(read_einzelbuchung).collect()
}
...
mod tests {
    ...
    fn test_read_einzelbuchungen() {
        ...
        assert_eq!(einzelbuchungen.get(0).unwrap().betrag, betrag(Vorzeichen::Negativ,123, 12));
        assert_eq!(einzelbuchungen.get(0).unwrap().kategorie, kategorie("NeueKategorie"));
        assert_eq!(einzelbuchungen.get(0).unwrap().name, name("Normal"));


        assert_eq!(einzelbuchungen.get(1).unwrap().datum, datum("2024-01-02"));
        assert_eq!(einzelbuchungen.get(1).unwrap().betrag, betrag(Vorzeichen::Negativ,122, 12));
        assert_eq!(einzelbuchungen.get(1).unwrap().kategorie, kategorie("NeueKategorie2"));
