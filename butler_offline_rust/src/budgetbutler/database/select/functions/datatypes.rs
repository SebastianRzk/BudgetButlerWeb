...
#[cfg(test)]
mod tests {
    use crate::budgetbutler::database::select::functions::datatypes::{EinnahmenAusgabenAggregation, MonatsAggregationsIndex};
    use crate::model::primitives::betrag::{Betrag, Vorzeichen};
    use std::cmp::Ordering;

    #[test]
    fn test_monats_aggregation_ord() {
        let a = MonatsAggregationsIndex { monat: 1, jahr: 2020 };
        let b = MonatsAggregationsIndex { monat: 2, jahr: 2020 };
