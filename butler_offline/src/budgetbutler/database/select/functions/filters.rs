...
pub fn filter_bis_einschliesslich_jahr<T: for<'a> BesitztDatum<'a>>(jahr: i32) -> impl Fn(&T) -> bool {
    move |item: &T| -> bool {
        item.datum().jahr <= jahr
    }
}
...
pub fn filter_auf_konto<T: for<'a>BesitztKontoReferenz<'a>>(konto_referenz: KontoReferenz) -> impl Fn(&T) -> bool {
    move |item: &T| -> bool {
        item.konto_referenz() == &konto_referenz
    }
}
...
#[cfg(test)]
mod tests {
    use super::{filter_auf_ausgaben, filter_auf_depot, filter_auf_einnahmen, filter_auf_konto, filter_auf_zeitraum};
    ...
    fn test_filter_bis_einschliesslich_jahr(){
        ...
        assert_eq!(filter(&einzelbuchung_with_datum(Datum::new(1, 1, 2020))), true);
        assert_eq!(filter(&einzelbuchung_with_datum(Datum::new(31, 12, 2020))), true);
        assert_eq!(filter(&einzelbuchung_with_datum(Datum::new(1, 1, 2021))), false);
    }

    #[test]
    fn test_filter_auf_depot(){
        assert_eq!(filter_auf_depot(&indiziert(Sparkonto { name: demo_name(), kontotyp: Kontotyp::Depot })), true);
        assert_eq!(filter_auf_depot(&indiziert(Sparkonto { name: demo_name(), kontotyp: Kontotyp::Sparkonto })), false);
        assert_eq!(filter_auf_depot(&indiziert(Sparkonto { name: demo_name(), kontotyp: Kontotyp::GenossenschaftsAnteile })), false);
    }
    ...
    fn test_filter_auf_konto(){
        ...
        assert_eq!(filter(&indiziert(order_with_konto(matching_konto_referenz))), true);
        assert_eq!(filter(&indiziert(order_with_konto(konto_referenz("as anderes")))), false);
    }
}