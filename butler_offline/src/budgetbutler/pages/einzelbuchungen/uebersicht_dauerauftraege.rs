...
pub fn handle_view(context: UebersichtDauerauftraegeContext) -> UebersichtDauerauftraegeViewResult {
    let result = context.database.dauerauftraege.select().group_as_list_by(start_ende_aggregation(context.today.clone()));

    UebersichtDauerauftraegeViewResult{
        aktuelle_dauerauftraege: result.get(&StartEndeAggregation::Aktuelle).unwrap_or(&vec![]).clone(),
        vergangene_dauerauftraege: result.get(&StartEndeAggregation::Vergangene).unwrap_or(&vec![]).clone(),
        zukuenftige_dauerauftraege: result.get(&StartEndeAggregation::Zukuenftige).unwrap_or(&vec![]).clone(),
