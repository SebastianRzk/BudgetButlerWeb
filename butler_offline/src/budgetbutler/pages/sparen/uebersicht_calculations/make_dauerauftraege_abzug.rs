...
pub fn make_dauerauftraege_abzuge(dauerauftraege: Vec<Indiziert<OrderDauerauftrag>>, konfigurierte_farben: Vec<Farbe>, database: &Database) -> Vec<DauerauftragAbzug>{
    let mut result = vec![];
    ...
    for dauerauftrag in dauerauftraege.iter(){
        let dauerauftrag_abzug = DauerauftragAbzug{
            ...
            depotwert_beschreibung: calc_depotwert_beschreibung(&dauerauftrag.value.depotwert.isin, database).description,
...
mod tests {
    ...
    fn test_make_dauerauftraege_abzuge_leer(){
        ...
        let result = super::make_dauerauftraege_abzuge(dauerauftraege, konfigurierte_farben, &database);
    ...
    fn test_make_dauerauftraege_abzuge(){
        let dauerauftraege = vec![
            indiziert(demo_order_dauerauftrag()),
        ];
        ...
        assert_eq!(result[0].depotwert_beschreibung, "Unbekannt (MeinDepotwert)");
