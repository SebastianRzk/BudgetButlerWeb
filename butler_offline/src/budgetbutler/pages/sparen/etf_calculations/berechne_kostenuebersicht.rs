use crate::budgetbutler::pages::sparen::uebersicht_etfs::{DepotwertMitDaten, ETFKosten, ETFKostenUebersicht};
...
pub fn berechne_kostenuebersicht(
    ...
    for depotwert_mit_kontostand in depotwerte_mit_kontostand.iter() {
        let kosten_prozent = Prozent::from_float_representation(depotwert_mit_kontostand.data.data.kosten);
        let kosten_euro = depotwert_mit_kontostand.aktueller_kontostand.anteil(&kosten_prozent);
...
mod tests {
    ...
    fn test_berechne_kostenuebersicht() {
        ...
        assert_eq!(result.gesamt.prozent, Prozent::from_str_representation("0.15"));
        assert_eq!(result.data.len(), 2);
        assert_eq!(result.data[0].euro, Betrag::new(Positiv, 2,50));
        assert_eq!(result.data[0].prozent, Prozent::from_str_representation("0.25"));
        assert_eq!(result.data[1].euro, betrag(2));
        assert_eq!(result.data[1].prozent, Prozent::from_str_representation("0.1"));
    }
}
