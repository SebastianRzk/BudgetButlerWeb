...
use crate::io::html::views::templates::chart_templates::{map_bar_chart_to_template, map_pie_chart, map_to_linechart_template, BarChartTemplate, LineChartTemplate, PieChartTemplate};
...
fn map_to_template(view_result: UebersichtSparenViewResult) -> UebersichtSparenTemplate {
    let diff = sum_to_german_string(view_result.anlagetypen.iter().map(|a| a.differenz.clone()).collect());
    let anlagetypen_gesamt = AnlagetypTemplate {
        color: "black".to_string(),
        name: "Gesamt".to_string(),
        gesamte_einzahlungen: sum_to_german_string(view_result.anlagetypen.iter().map(|a| a.gesamte_einzahlungen.clone()).collect()),
        difference_is_negativ: diff.starts_with("-"),
        difference: diff,
        kontostand: sum_to_german_string(view_result.anlagetypen.iter().map(|a| a.kontostand.clone()).collect()),
    };
    UebersichtSparenTemplate {
        einnahmen_ausgaben_sparen: EinnahmenAusgabenSparenTemplate{
            einnahmen: JSONBetragList::new(view_result.einnahmen_ausgaben_sparen.iter().map(|e| e.einnahmen.clone()).collect()),
            ausgaben: JSONBetragList::new(view_result.einnahmen_ausgaben_sparen.iter().map(|e| e.ausgaben.abs()).collect()),
            sparen: JSONBetragList::new(view_result.einnahmen_ausgaben_sparen.iter().map(|e| e.sparen.abs()).collect()),
            labels: JSONStringList::new(view_result.einnahmen_ausgaben_sparen.iter().map(|e| format!("{}", e.jahr)).collect()),
        },
        ...
        gesamt_entwicklung_delta_jahr: map_bar_chart_to_template(view_result.gesamt_entwicklung_delta_jahr),
        depot_metainfos: view_result.depot_metainfos.iter().map(|d| DepotMetaInfoTemplate{
        ...
        uebersicht_kontos: UebersichtKontosTemplate{
            konten: view_result.uebersicht_kontos.konten.iter().map(|k| SparkontoMitFarbeTemplate{
            ...
            aufbuchungen: view_result.uebersicht_kontos.aufbuchungen.to_german_string(),
        ...
        anlagetypen: view_result.anlagetypen.iter().map(|a| AnlagetypTemplate{
        ...
        aktuelle_dauerauftraege: view_result.aktuelle_dauerauftraege.iter().map(|a| OrderDauerauftragAbstractTemplate{
            color: a.farbe.as_string.clone(),
...
fn sum_to_german_string(betrag_list: Vec<Betrag>) -> String {
    betrag_list.into_iter().reduce(|x, y| x + y).unwrap_or(Betrag::zero()).to_german_string()
}

fn optional_datum(d: &Option<Datum>) -> String {
    d.clone().map(|dd| dd.to_german_string()).unwrap_or("Noch nichts erfasst".to_string())
}
