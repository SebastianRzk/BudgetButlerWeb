use askama::Template;
use crate::budgetbutler::pages::sparen::uebersicht_etfs::{ETFKosten, EtfInfo, Tabelle, TabellenZeile, TabellenZelle, UebersichtEtfViewResult};
...
fn map_to_template(view_result: UebersichtEtfViewResult) -> UebersichtEtfsTemplate {
    UebersichtEtfsTemplate {
        etfs: view_result.etfs.iter().map(|x| map_etf_to_template(x)).collect(),
        kosten: ETFKostenUebersichtTemplate {
            gesamt: map_etf_kosten_to_template(&view_result.etfkosten.gesamt),
            data: view_result.etfkosten.data.iter().map(|x| map_etf_kosten_to_template(x)).collect()
        },
...
fn map_tabelle_to_template(tabelle_template: Tabelle) -> TabelleTemplate {
    TabelleTemplate {
        ...
        rows: tabelle_template.rows.iter().map(|x| map_tabellenzeile_to_template(x)).collect()
    }
}
...
fn map_tabellenzeile_to_template(tabellen_zeile_template: &TabellenZeile) -> TabellenZeileTemplate {
    TabellenZeileTemplate {
        ...
        other_columns: tabellen_zeile_template.other_columns.iter().map(|x| map_tabellenzelle_to_template(x)).collect()
    }
}
