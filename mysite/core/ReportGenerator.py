...
class ReportGenerator():
    ...
    def generate_pages(self):
        content_rows_per_page = (self._row_count - 2 - 2) * 2
        page = 1
        full_rows = math.ceil(len(self._half_lines) / 2) + len(self._full_lines)
        max_pages = math.ceil(full_rows / content_rows_per_page)
        for i in range(0, len(self._half_lines), content_rows_per_page):
            self._add_header()
            for element in range(i, i + math.ceil(content_rows_per_page/2)):
                left = self._get_half_line(element)
                right = self._get_half_line(math.ceil(element + content_rows_per_page/2))
